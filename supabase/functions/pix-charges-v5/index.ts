...
async function confirmarPagamento(admin: any, link: Record<string, unknown>, btgPayload: Record<string, unknown>) {
  ...
  // Guarda atômica anti-corrida: só o caller que transiciona a linha para PAGO
  // segue para baixa+notificação. Página em duas abas, webhook e poll podem
  // confirmar simultaneamente — o banco decide quem venceu.
  const { data: claimed } = await admin.from("payment_links").update({
    status: "PAGO",
  ...
  }).eq("id", link.id).neq("status", "PAGO").select("id");
  if (!claimed || claimed.length === 0) return { already: true };
      ...
      case "criar": {
        ...
        if (isSandbox) {
          ...
          const btgPayload: Record<string, unknown> = {
            ...
            // OpenAPI oficial: o campo do valor na CRIAÇÃO é amount.original
            // (amount.value só aparece no payload de webhook).
            amount: {
              original: Number(valorNum.toFixed(2)),
