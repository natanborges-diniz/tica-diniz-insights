-- Provisionamento por rubrica: a rubrica é permanente; os lançamentos PREVISTO
-- do horizonte (12m rolantes) nascem dela, idempotentes por competência.
ALTER TABLE public.rubricas_autorizadas
  ADD COLUMN IF NOT EXISTS dia_vencimento INTEGER,      -- 1..28 (default 10 no gerador)
  ADD COLUMN IF NOT EXISTS provisionar BOOLEAN NOT NULL DEFAULT true;

ALTER TABLE public.lancamentos_financeiros
  ADD COLUMN IF NOT EXISTS competencia_rubrica TEXT;    -- 'YYYY-MM'

-- Idempotência do gerador (rubrica global provisiona por empresa)
...
-- Horizonte rolante: todo dia 1 às 12:00 UTC (09:00 BRT)
