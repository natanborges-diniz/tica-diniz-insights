-- Espelha a estrutura de lojas da REDE para a CIELO.
--
-- O vinculo anterior foi construido sobre uma premissa errada: que um CNPJ
-- correspondia a uma loja. Na Cielo cada "Estabelecimento" e um ponto de venda,
-- e uma empresa pode ter varias lojas sob o mesmo CNPJ — foi por isso que
-- Jandira, Itapevi e Barueri ficaram de fora: todos os PVs de cada CNPJ caiam
-- na primeira loja que casava.
--
-- Aqui a Cielo passa a ter uma linha por loja, igual a REDE, com o CNPJ
-- preenchido a partir do cadastro que ja existe. Os PVs ficam VAZIOS de
-- proposito: sao preenchidos a mao em Admin > Adquirentes, porque so quem
-- conhece as lojas sabe qual maquininha esta em qual endereco.
--
-- O que ja foi preenchido a mao e preservado: matriz de extrato e chave HMAC
-- nao sao tocadas.

-- ---------------------------------------------------------------------------
-- 1. Uma configuracao CIELO para cada loja que tem REDE
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 2. CNPJ vindo do cadastro que ja esta em uso
-- ---------------------------------------------------------------------------

-- Preferencia para o CNPJ da propria loja; o cadastro bancario entra quando
-- empresa.cnpj ainda estiver vazio.
...
-- ---------------------------------------------------------------------------
-- 3. Zera os PVs distribuidos pela premissa errada
-- ---------------------------------------------------------------------------

-- Deixar os PVs onde estao seria pior do que deixa-los vazios: venda cairia na
-- loja errada em silencio, e a tela nao daria nenhum sinal disso. Vazio, a
-- importacao acusa "estabelecimento sem loja associada" e mostra exatamente
-- qual PV falta cadastrar.
...
-- ---------------------------------------------------------------------------
-- 4. Referencia dos PVs da planilha, para consultar ao preencher
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 5. Situacao para conferir na tela
-- ---------------------------------------------------------------------------
