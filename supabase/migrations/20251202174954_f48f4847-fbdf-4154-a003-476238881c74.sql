-- Corrigir função para incluir mes_nome e dia_semana_nome
...
DECLARE
  ...
  meses_nomes text[] := ARRAY['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'];
  dias_nomes text[] := ARRAY['Domingo','Segunda','Terça','Quarta','Quinta','Sexta','Sábado'];
BEGIN
  ...
  INSERT INTO dw.dim_produto (id_produto, cod_produto, descricao, categoria, referencia)
  SELECT cod_produto, cod_produto, descricao, categoria, referencia
  FROM public.produto
  ...
  INSERT INTO dw.dim_tempo (id_tempo, data, ano, mes, dia, mes_nome, dia_semana, dia_semana_nome, trimestre)
  SELECT DISTINCT
    ...
    meses_nomes[EXTRACT(MONTH FROM data_emissao)::int] AS mes_nome,
    EXTRACT(DOW FROM data_emissao)::int AS dia_semana,
    dias_nomes[EXTRACT(DOW FROM data_emissao)::int + 1] AS dia_semana_nome,
