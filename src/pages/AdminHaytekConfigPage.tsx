...
import { Loader2, Box, Save, RefreshCw } from "lucide-react";
...
export default function AdminHaytekConfigPage() {
  ...
  const totalConfiguradas = configs.filter((c) => c.store_id && c.cnpj).length;
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-2xl font-bold">Configuração Haytek por Empresa</h1>
            <p className="text-sm text-muted-foreground">
              Mapeamento de CNPJ, Store ID e Address ID Haytek para cada unidade
            </p>
          </div>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Insira o CNPJ, Store ID e Address ID Haytek para cada empresa. O alias é o nome de exibição usado nos pedidos.
          </CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="w-36">Alias / Nome</TableHead>
              ...
              <TableBody>
                {configs.map((config) => {
                  const row = editing[config.id] || { cnpj: "", store_id: "", address_id: "", alias: "" };
                  const changed = isRowChanged(config);
                  const isSaving = saving === config.id;
                  const configurada = config.store_id && config.cnpj;
